...
//!
//! The host executes skills; agent-core defines the contracts and validates outputs.
...
impl ExtractionOutput {
    ...
    /// Create output for name extraction
    pub fn names(names: Vec<String>) -> Self {
        Self {
            result: serde_json::json!({ "name": names }),
        }
    }
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::HallucinationDetected(val) => {
                write!(
                    f,
                    "HallucinationDetected: '{}' not found in source text",
                    val
                )
            }
...
pub fn is_valid_skill(name: &str) -> bool {
    AVAILABLE_SKILLS.iter().any(|s| s.name == name)
}
...
/// It ensures:
/// 1. Output contains the expected target field
/// 2. Extracted values appear in the source text (no hallucination)
pub fn validate_extraction_output(
    ...
    match target {
        ...
        ExtractionTarget::Name => {
            // For names, use lenient matching (check individual words)
            if let Some(values) = output.result.get("name") {
                let items: Vec<&str> = match values {
                    Value::String(s) => vec![s.as_str()],
                    Value::Array(arr) => arr.iter().filter_map(|v| v.as_str()).collect(),
                    _ => vec![],
                ...
                for item in items {
                    // More lenient matching for names (check individual words)
...
/// Apply a skill result to the agent state
///
/// This adds the skill result to the conversation history so the model
/// can see what happened when it invoked the skill.
pub fn format_skill_result(result: &SkillResult_) -> String {
    if result.success {
        format!("Skill output:\n{}", result.to_json())
    } else {
        format!(
...
mod tests {
    ...
    fn test_extraction_target_from_str() {
        assert_eq!(
            ExtractionTarget::from_str("email"),
            Some(ExtractionTarget::Email)
        );
        assert_eq!(
            ExtractionTarget::from_str("URL"),
            Some(ExtractionTarget::Url)
        );
    ...
    fn test_input_validation() {
        ...
        assert!(matches!(
            ...
            Err(SkillError::InvalidTarget(_))
        ));
    }
    ...
    fn test_is_valid_skill() {
        assert!(is_valid_skill("extract"));
        assert!(!is_valid_skill("unknown"));
    }
}
